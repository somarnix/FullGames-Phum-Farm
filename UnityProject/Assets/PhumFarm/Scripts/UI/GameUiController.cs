...
namespace PhumFarm.UI
{
    public sealed partial class GameUiController : MonoBehaviour
    {
        ...
        private void BuildHud()
        {
            ...
            GameObject status = Panel(hud.transform, "PlayerStatus", wood, new Vector2(0, 1), new Vector2(0, 1), new Vector2(16, -92), new Vector2(315, -16));
            Label(status.transform, "Portrait", "PF", 24, FontStyle.Bold, TextAnchor.MiddleCenter, Vector2.zero, new Vector2(0, 1), new Vector2(9, 9), new Vector2(66, -9), gold);
            statusText = Label(status.transform, "Status", string.Empty, 17, FontStyle.Bold, TextAnchor.MiddleLeft, Vector2.zero, Vector2.one, new Vector2(78, 7), new Vector2(-12, -7), cream);

            GameObject time = Panel(hud.transform, "Time", wood, new Vector2(.5f, 1), new Vector2(.5f, 1), new Vector2(-125, -70), new Vector2(125, -16));
            clockText = Label(time.transform, "Clock", string.Empty, 18, FontStyle.Bold, TextAnchor.MiddleCenter, Vector2.zero, Vector2.one, new Vector2(8, 4), new Vector2(-8, -4), cream);

            GameObject currency = Panel(hud.transform, "Currency", wood, Vector2.one, Vector2.one, new Vector2(-365, -70), new Vector2(-82, -16));
            currencyText = Label(currency.transform, "CurrencyText", string.Empty, 18, FontStyle.Bold, TextAnchor.MiddleCenter, Vector2.zero, Vector2.one, new Vector2(8, 4), new Vector2(-8, -4), cream);
            MakeButton(hud.transform, "Pause", "MENU", Vector2.one, Vector2.one, new Vector2(-72, -70), new Vector2(-16, -16), WorldPrimitiveFactory.Hex("704A42"), ShowPauseMenu);

            GameObject quest = Panel(hud.transform, "Quest", new Color(cream.r, cream.g, cream.b, .88f), new Vector2(0, 1), new Vector2(0, 1), new Vector2(16, -205), new Vector2(290, -108));
            questText = Label(quest.transform, "QuestText", string.Empty, 15, FontStyle.Bold, TextAnchor.MiddleLeft, Vector2.zero, Vector2.one, new Vector2(15, 8), new Vector2(-35, -8), WorldPrimitiveFactory.Hex("493226"));
            ...
            GameObject prompt = Panel(hud.transform, "Prompt", wood, new Vector2(.5f, 0), new Vector2(.5f, 0), new Vector2(-250, 20), new Vector2(250, 66));
            promptText = Label(prompt.transform, "Text", string.Empty, 16, FontStyle.Bold, TextAnchor.MiddleCenter, Vector2.zero, Vector2.one, new Vector2(12, 4), new Vector2(-12, -4), cream);

            GameObject nav = Panel(hud.transform, "BottomNavigation", new Color(wood.r, wood.g, wood.b, .35f), Vector2.zero, Vector2.zero, new Vector2(16, 16), new Vector2(436, 76));
            string[] names = { "SHOP", "BUILD", "ORDERS", "BAG", "MAP" };
            Action[] actions = { ShowShop, ShowBuildMode, ShowOrders, ShowInventory, ShowMap };
            for (int i = 0; i < names.Length; i++)
            {
                int captured = i;
                MakeButton(nav.transform, names[i], names[i], Vector2.zero, new Vector2(0, 1), new Vector2(5 + i * 82, 5), new Vector2(79 + i * 82, -5), i == 2 ? terracotta : green, () => actions[captured]());
            }

            MakeButton(hud.transform, "ContextAction", "ACTION", Vector2.one, Vector2.one, new Vector2(-102, -166), new Vector2(-18, -94), terracotta,
                () => world?.Player?.InteractNearest());

            toastText = Label(hud.transform, "Toast", string.Empty, 20, FontStyle.Bold, TextAnchor.MiddleCenter, new Vector2(.5f, 1), new Vector2(.5f, 1), new Vector2(-280, -135), new Vector2(280, -88), WorldPrimitiveFactory.Hex("FFF2B3"));
        ...
        private void Refresh()
        {
            ...
            currencyText.text = $"COINS {data.economy.coins:N0}     GEMS {data.economy.gems}";

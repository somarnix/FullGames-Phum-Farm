...
namespace PhumFarm.Core
{
    public sealed class GameState
    {
        ...
        public GameSaveData CreateDefault()
        {
            ...
            foreach (string itemId in new[] { "wheat", "rice", "corn", "tomato", "sugarcane", "egg", "milk", "flour", "animal_feed", "bread", "butter", "palm_sugar" })
            {
        ...
        public void Normalize()
        {
            ...
            if (Data.inventory.capacity <= 0) Data.inventory.capacity = balance.barnCapacity;
            if (Data.production.queueSlots <= 0) Data.production.queueSlots = 3;
        ...
        public void AdvanceGameMinutes(double minutes, bool notify = true)
        {
            if (minutes <= 0d) return;
            double total = Data.world.minutes + minutes;
            Data.world.day += (int)(total / 1440d);
            Data.world.minutes = (float)(total % 1440d);
            if (notify) NotifyChanged();
        }

        public void AdvanceTime(float realSeconds)
        {
            int previousDay = Data.world.day;
            AdvanceGameMinutes(Math.Max(0f, realSeconds) * 3f, false);
            if (Data.world.day != previousDay) GameServices.Instance?.Saves.Save();
        }

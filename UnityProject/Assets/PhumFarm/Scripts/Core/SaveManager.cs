...
namespace PhumFarm.Core
{
    public sealed class SaveManager
    {
        ...
        private readonly TimeSystem time;
        ...
        public SaveManager(GameState state, string overridePath = null, TimeSystem time = null)
        {
            this.state = state;
            this.time = time;
        ...
        public bool Load()
        {
            ...
            foreach (string candidate in new[] { savePath, BackupPath })
            {
                ...
                try
                {
                    ...
                    Migrate(loaded);
                    long savedAt = loaded.savedAtUnix;
                    state.Replace(loaded);
                    time?.ApplyOfflineProgress(savedAt);
        ...
        private static void Migrate(GameSaveData value)
        {
            ...
            {
                value.settings ??= new SettingsState();
                value.inventory ??= new InventoryState();
                if (value.inventory.capacity <= 0) value.inventory.capacity = 75;
                value.production ??= new ProductionState();
                if (value.production.queueSlots <= 0) value.production.queueSlots = 3;
            }
            if (value.saveVersion < 4)
            {
                value.orders ??= new OrderState();
                foreach (BuildingPlacement placement in value.buildings?.placements ?? new System.Collections.Generic.List<BuildingPlacement>())
                    if (string.IsNullOrEmpty(placement.instanceId)) placement.instanceId = Guid.NewGuid().ToString("N");
                foreach (AnimalJob animal in value.animals?.jobs ?? new System.Collections.Generic.List<AnimalJob>())
                    if (string.IsNullOrEmpty(animal.phase)) animal.phase = animal.fed ? "producing" : "hungry";
            }

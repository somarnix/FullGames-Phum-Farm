...
namespace PhumFarm.Core
{
    public sealed class UnlockSystem
    {
        ...
        public bool Recipe(string id) => balance.Product(id) is { } value && Level(value.unlockLevel);
        public bool Building(string id) => balance.catalog?.Building(id) is { } value && Level(value.unlockLevel);

...
namespace PhumFarm.World
{
    public sealed class FarmWorldController : MonoBehaviour
    {
        ...
        public BuildingPlacementSystem Placement { get; private set; }
        ...
        private void Update()
        {
            GameServices.Instance.Time.Tick(Time.deltaTime);
            if (Time.frameCount % 60 == 0) GameServices.Instance.Orders.RefreshExpired();
        }
        ...
        private void Build()
        {
            ...
            RefreshExpansions();
            Placement = gameObject.AddComponent<BuildingPlacementSystem>();
        ...
        {
            SetDistrict<AnimalDistrict>(GameServices.Instance.Unlocks.Expansion("animal_meadow"));
            SetDistrict<ProductionDistrict>(GameServices.Instance.Unlocks.Expansion("production_village"));
            SetDistrict<RiverOrchardDistrict>(GameServices.Instance.Unlocks.Expansion("river_orchard"));
        }

        private void SetDistrict<T>(bool active) where T : FarmDistrict
        {
            T district = GetComponentInChildren<T>(true);
            if (district != null) district.gameObject.SetActive(active);
        }
